
{{-- recibo la variable en el componente --}}
@props(['post'])


{{-- copio el codigo desde el archivo category.blade.php --}}
<article class="mb-8 bg-white shadow-lg rounded-lg overflow-hidden">
    @if ($post->image)
    <img class="w-full h-72 object-cover object-center" src="{{ asset(Storage::url($post->image->url))}}">
    @else
    <img class="w-full h-72 object-cover object-center" src="https://cdn.pixabay.com/photo/2021/08/10/18/32/cat-6536684_1280.jpg">
        
    @endif

<div class="px-6 py-4">
    <h1 class="font-bold text-xl mb-2"></h1>
        <a href="{{route('posts.show', $post)}}">{{$post->name}}</a>

<div class="text-gray-700 text-base">
 {!!$post->extract!!}
</div>
</div>

<div class="px-6 pt-4 pb-2">

    @foreach ($post->tags as $tag)
        <a href="{{route('posts.tag', $tag)}}" class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm text-gray-700 mr-2 ">{{$tag->name}}</a>
    @endforeach

</div>

</article>